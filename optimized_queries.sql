...
ON t.l_partkey = l.l_partkey
WHERE
    p_brand = 'Brand#52'
    AND p_container = 'JUMBO CAN'
    AND l_quantity < t.threshold;
Runtime:36.8

Optimized 20:
WITH eligible_suppliers AS (
    SELECT
        ps_suppkey
    FROM
        partsupp ps
...
valid_parts AS (
    ...
    FROM
        partsupp ps
    WHERE
        ps.ps_suppkey IN (SELECT ps_suppkey FROM eligible_suppliers)
),
...
JOIN 
    part p ON p.p_partkey = l.l_partkey
JOIN
    filtered_lineitem fl ON fl.l_partkey = l.l_partkey
WHERE
    p.p_brand = 'Brand#52'
    AND p.p_container = 'JUMBO CAN'
    AND l.l_quantity < (0.2 * fl.avg_qty);
Runtime:36.9
Optimized 22:
WITH base AS (
    SELECT
